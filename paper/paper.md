...
Users can select the classification values to be included in the initial photon selection for the ATL06-SR fitting. For example, a user might wish to generate ATL06-SR products based only on photons with ATL03 photon classification values indicating medium or better confidence, or photons flagged as ground returns in ATL08 to remove photons from canopy returns. The rapid processing offered by the SlideRule service allows for interactive testing and evaluation of various combinations of ATL06-SR parameter and classificaiton schemes for user- and application-specific needs.
